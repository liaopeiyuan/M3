function [workspace,cuboid] = workspace(m,position,orientation,params)
    ...
    if and(max(size(position))>3, max(size(position))>3)
        if opt.verbose
        ...
        H = cuboid(xmin,xmax,ymin,ymax,zmin,zmax,deltaX,deltaY,deltaZ);
        ...
        H2 = cuboid(xmin,xmax,ymin,ymax,zmin,zmax,deltaX,deltaY,deltaZ);
    ...
    if not(exist('H2'))
        for k=1:sizeZ
           for j=1:sizeY
              for i=1:sizeX  
                 try
                    q=m.inverse(fDim1(H{i,j,k}(1),H{i,j,k}(2),H{i,j,k}(3))*fDim2(dim2(1),dim2(2),dim2(3)));
                    if all(not(isnan(q)))
              ...
              for i=1:sizeX
                 for k1=1:sizeZ1
                    for j1=1:sizeY1
                    	for i1=1:sizeX1 
                             try
                                q=m.inverse(fDim1(H{i,j,k}(1),H{i,j,k}(2),H{i,j,k}(3))*fDim2(H2{i,j,k}(1),H2{i,j,k}(2),H2{i,j,k}(3)));
                                if all(not(isnan(q)))
